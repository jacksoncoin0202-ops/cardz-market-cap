...
# The pipeline authority is a different branch/tree; run its no-DB suite too.
# Otherwise a green origin/main release can publish while every authority-only
# collector/orchestrator regression test is absent from the checkout being tested.
...
"$TEST_PY" -X utf8 "$SOURCE_REPO/scripts/run_all_tests.py" --no-db --skip-fe
