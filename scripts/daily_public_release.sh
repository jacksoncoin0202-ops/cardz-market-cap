...
# The release checkout is the exact tree that will bake. Run every no-DB guard
# from that tree after the fast-forward and before the first generated byte, so a
# red guard cannot be bypassed by a green Windows working tree or a stale WSL
# checkout. This venv is the documented WSL backend runtime and already carries
# the Python dependencies imported by the pipeline self-tests.
