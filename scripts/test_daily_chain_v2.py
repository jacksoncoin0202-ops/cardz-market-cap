...
with tempfile.TemporaryDirectory(prefix="v2-o2-fixf-") as folder:
    ...
    for _ in range(80):
        ...
        if "daily-accept" in o2f_order:
            break
        # closing(): sqlite3's own context manager commits but leaves the
        # handle open, and on Windows an open handle makes the enclosing
        # TemporaryDirectory cleanup raise after every check has passed.
        with closing(sqlite3.connect(str(o2f_journal.path))) as o2f_conn:
            o2f_conn.execute(
