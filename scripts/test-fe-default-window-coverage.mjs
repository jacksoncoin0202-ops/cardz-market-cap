 ...
 * 舊預設 180d 喺 08-23 嗰份 seed snapshot 上面只有 165/1604（10.3%）張卡搵到錨 —— 即係
 ...
 *  ③ 攞 `data/public/seed-snapshot.json` 全板 1,604 張卡跑真 `windowMetrics`（history 收窄成
 *     成交日，即係 R6 之後個 series），配置嘅 `defaultMarketWindow` 要 ≥ 60% 有錨。
 *     有人揼返去 180d（10.3%）就紅；日後成交數據長多咗，長窗自然過到，呢條閘會自己放行。
...
/*
 * R6 之後條 series = 「有成交嗰日」。seed snapshot 係 R6 之前 bake 嘅，所以要喺度收窄返：
 * 冇成交嘅日子（K 線點）踢走，有成交嗰日個價 = 當日成交均價（同 producer 而家嘅寫法一樣）。
...
const anchored = (card, code) => windowMetrics(
...
const coverage = (cards, code) => (cards.length === 0
  ? 0
  : cards.filter((card) => anchored(card, code)).length / cards.length);
 ...
 * ───────────────────────────────────────────────────────────── */
{
  ...
  const table = typesLib.marketWindows.map((code) => {
    const hit = cards.filter((card) => anchored(card, code)).length;
    return { code, hit, pct: cards.length === 0 ? 0 : hit / cards.length };
  });
  const line = table
    .map(({ code, hit, pct }) => `${code}=${hit}/${cards.length} (${(pct * 100).toFixed(1)}%)`)
    .join("  ");
  console.log(`[anchor coverage] ${line}`);

  const chosen = table.find((row) => row.code === DEFAULT_WINDOW);
