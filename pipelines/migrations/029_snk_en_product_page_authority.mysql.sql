-- Actual SNK EN product-page authority for canonical default images.
-- Additive on top of 026: old storefront/image rows remain historical but are
-- not projectable until an exact page payload is hash-bound to the lineage.

CREATE TABLE IF NOT EXISTS market_snk_en_product_page_authority (
    id BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
    storefront_lineage_id BIGINT UNSIGNED NOT NULL,
    variant_id BIGINT UNSIGNED NOT NULL,
    exact_item_id VARCHAR(191) NOT NULL,
    product_url VARCHAR(1000) NOT NULL,
    final_url VARCHAR(1000) NOT NULL,
    http_status SMALLINT UNSIGNED NOT NULL,
    product_page_payload_sha256 CHAR(64) NOT NULL,
    product_page_observed_at DATETIME(6) NOT NULL,
    ...
    PRIMARY KEY (id),
    UNIQUE KEY uq_snk_en_page_lineage (storefront_lineage_id),
    UNIQUE KEY uq_snk_en_page_authority (authority_sha256),
    KEY ix_snk_en_page_variant (variant_id, product_page_observed_at),
    KEY ix_snk_en_page_item (exact_item_id, product_page_observed_at),
    CONSTRAINT fk_snk_en_page_lineage FOREIGN KEY (storefront_lineage_id)
        REFERENCES market_snk_en_storefront_lineage(id),
    CONSTRAINT fk_snk_en_page_variant FOREIGN KEY (variant_id)
        REFERENCES catalog_variant(id)
...
INSERT IGNORE INTO cardz_schema_version (version_code) VALUES ('029');
