-- Product projection runtime correction.
--
-- Migration 024 made the daily fact projection canonical, but its product
-- view evaluated that full price/POP/sales history three separate times to
-- obtain three latest rows.  On the live Windows 3308 dataset this exhausted
-- the old 128MB buffer pool and still exceeded 120 seconds after the pool was
-- corrected.  Keep the daily projection as the history authority and resolve
-- each current product metric directly from the same exact-source policies.
...
eligible_price_current AS (
  SELECT
    p.id,p.variant_id,p.observed_date,p.price_usd,
    CASE WHEN p.source_code='ebay' AND r.run_key LIKE '%tpl%'
      THEN 'tcgpricelookup' ELSE p.source_code END AS price_source_code,
    ...
    ROW_NUMBER() OVER (
      PARTITION BY p.variant_id
      ORDER BY
        p.observed_date DESC,
        CASE
...
latest_price AS (
  SELECT * FROM eligible_price_current WHERE metric_rank=1
),
eligible_population_current AS (
  SELECT
    pop.id,pop.variant_id,pop.source_code AS population_source_code,
    ...
    ROW_NUMBER() OVER (
      PARTITION BY pop.variant_id
      ORDER BY pop.observed_date DESC,pop.effective_at DESC,pop.id DESC
    ) AS metric_rank
...
latest_population AS (
  SELECT * FROM eligible_population_current WHERE metric_rank=1
),
transaction_sales AS (
  SELECT
    ...
    CASE WHEN MIN(s.coverage_status='complete')=1 THEN 'complete' ELSE 'partial' END AS sales_coverage_status,
    MAX(s.fetched_at) AS sales_evidence_at
...
verified_zero_sales AS (
  SELECT
    ...
    'complete' AS sales_coverage_status,MAX(r.completed_at) AS sales_evidence_at
  ...
  WHERE a.sales_count=0 AND a.coverage_status='complete'
    AND EXISTS (
      ...
      WHERE si.variant_id=a.variant_id
        AND si.source_code=CASE WHEN a.source_code IN ('snk','snk_psa10') THEN 'snkrdunk' ELSE a.source_code END
        AND LOWER(si.match_status)='exact'
    )
    AND NOT EXISTS (
...
latest_sales AS (
  SELECT ranked.* FROM (
    SELECT d.*,ROW_NUMBER() OVER (
      PARTITION BY d.variant_id ORDER BY d.observed_date DESC,d.sales_evidence_at DESC
    ) AS metric_rank
    FROM daily_sales d
...
INSERT IGNORE INTO cardz_schema_version (version_code) VALUES ('025');
