...
def assert_canonical_db(connection, port: int) -> None:
    with connection.cursor() as cur:
        ...
        found = {str(next(iter(row.values()))) for row in cur.fetchall()}
    missing = [t for t in CANONICAL_DB_MARKER_TABLES if t not in found]
    if missing:
        raise RuntimeError(
            f"refusing to run: CARDZ_DB_PORT={port} is not the canonical New-Era "
            f"database (missing {', '.join(missing)}). Canonical is 3308."
        )
...
def db():
    ...
    if not raw_port:
        raise RuntimeError(
            "CARDZ_DB_PORT is required and has no default. Set it in "
