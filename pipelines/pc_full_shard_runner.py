...
def process_one(work: dict[str, Any], force: bool = False) -> dict[str, Any]:
    ...
    for url in urls[:4]:
        ...
        claim = {
            "pc_product_id": int(product_id) if product_id.isdigit() else product_id,
            "title": title,
            "collector": work.get("num"),
            "set": work.get("set"),
            "cf_path": "cdp_9222",
            "psa10_sales_count": sold.get("count") or len(rows),
            "psa10_price_range_usd": [min(prices), max(prices)] if prices else None,
            "sample_titles": [row.get("title") for row in rows[:2]],
            "note": "PC full900 tracked shard runner",
        }
        match_status = "verified" if rows else "candidate"
        with db() as connection:
            cursor = connection.cursor()
            cursor.execute(
                """
                SELECT
                    identity.variant_id AS owner_variant_id,
                    COALESCE(alias.canonical_variant_id, identity.variant_id)
                        AS owner_canonical_variant_id
            ...
            cursor.execute(
                """
                SELECT COALESCE(alias.canonical_variant_id, %s)
                    AS canonical_variant_id
                FROM (SELECT %s AS variant_id) AS target
                LEFT JOIN catalog_variant_alias AS alias
                  ON alias.duplicate_variant_id = target.variant_id
                """,
                (variant_id, variant_id),
            )
            target_canonical = int(cursor.fetchone()["canonical_variant_id"])
            owner_canonical = (
                int(owner["owner_canonical_variant_id"]) if owner is not None else None
            )
            identity_conflict = (
                owner_canonical is not None and owner_canonical != target_canonical
            )
            evidence_sha256 = upsert_evidence(
                cursor,
                variant_id=variant_id,
                evidence_kind="external_url",
                source_code="pricecharting",
                external_entity_id=product_id,
                external_url=final_url,
                match_status="conflict" if identity_conflict else match_status,
                claim=claim,
                actor="agent_pc_full900",
            )
            if identity_conflict:
                connection.commit()
                item.update(
                    {
                        "status": "identity_owner_conflict",
                        "product_id": product_id,
                        "title": title,
                        "url_final": final_url,
                        "owner_variant_id": int(owner["owner_variant_id"]),
                        "owner_canonical_variant_id": owner_canonical,
                    }
                )
                return item
            if rows and owner is None:
                cursor.execute(
                    ...
                    VALUES ('pricecharting', %s, %s, 'exact', %s)
                    """,
                    (product_id, variant_id, evidence_sha256),
                )
            elif rows and int(owner["owner_variant_id"]) == variant_id:
                cursor.execute(
                    """
                    ...
                    WHERE source_code='pricecharting'
                      ...
                      AND variant_id=%s
                    """,
                    (evidence_sha256, product_id, variant_id),
                )
            connection.commit()
