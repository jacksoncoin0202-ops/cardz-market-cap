# CARDZ Market Cap — Archived Project Rules (2026-07-29)
...
## 價格源（SNK = 日文榜；US eBay 衍生 = TPL SSR，無 API key）

規矩：[docs/US_PRICE_SOURCE_RULES.md](docs/US_PRICE_SOURCE_RULES.md)

| 檔 | 做咩 |
|---|---|
| [pipelines/snk_market_data.py](pipelines/snk_market_data.py) | **PSA10 日文榜價主線**（SNK） |
| [pipelines/tcgpricelookup_ssr.py](pipelines/tcgpricelookup_ssr.py) | **US eBay PSA10 + ~1y 日史（SSR，無 key）** full/incremental |
| [pipelines/us_price_fallback.py](pipelines/us_price_fallback.py) | TPL → TCGFish → Collectr fallback 鏈 |
| [pipelines/pricecharting_ebay_export.py](pipelines/pricecharting_ebay_export.py) | eBay **逐筆 sold**（PC 頁 → ebay_sold input） |
| [pipelines/c11_pc_sold_ingest.py](pipelines/c11_pc_sold_ingest.py) | C11 map HTML → `market_sale_observation`（`pc:{product_id}`；**唔寫** ebay identity） |
| [pipelines/ebay_sold_data.py](pipelines/ebay_sold_data.py) | PSA10 sold 嚴格 normalizer |
| [pipelines/ebay_brute_harvest.py](pipelines/ebay_brute_harvest.py) | eBay item（sold search 仍 PX 擋） |
...
| 檔 | 做咩 |
|---|---|
| [pipelines/tcgplayer_images.py](pipelines/tcgplayer_images.py) | **主線** TCGplayer search + CDN |
| [pipelines/native_image_resolver.py](pipelines/native_image_resolver.py) | 規格權威；priority = TCGplayer → SNK/G10 fallback → Kado |
| [pipelines/snk_image_ingest.py](pipelines/snk_image_ingest.py) | **SNK live 原圖落地 only**：`pending_review` / `public_allowed=0`；multi-SNK reject。**唔** auto 出街。Plan：[docs/SNK_IMAGE_PIPELINE.md](docs/SNK_IMAGE_PIPELINE.md) |
| [pipelines/snk_image_promotion.py](pipelines/snk_image_promotion.py) | **SNK 審批 hard gate**：`enroll-from-qc`（QC marketRank）→ re-verify approve（禁 bypass、multi-SNK、printing 綁 fingerprint）→ receipt → manifest。Tests：`tests/test_snk_image_promotion.py`。見 [docs/SNK_IMAGE_PROMOTION.md](docs/SNK_IMAGE_PROMOTION.md) |
| [pipelines/printing_review_batch.py](pipelines/printing_review_batch.py) | **Step 4 printing field review**：`open` ranked worksheets → 人手／vision 填 evidence → `seal-worksheet` → `printing-decisions/<sha>`。**唔** materialize DB。見 [docs/CANONICAL_PRINTING_MATERIALIZATION.md](docs/CANONICAL_PRINTING_MATERIALIZATION.md) |
