# Archived 項目地圖 + 逆向／取數「點做」手冊 (2026-07-29)

> **點用**：腳本多、分支多——呢份係**地圖**。每源只寫 **點做**，少講方法論。  
> **唯一營運必讀**仍係 [`PROJECT_STATE.md`](../PROJECT_STATE.md)。呢份係 **地圖／逆向附錄**。  
> 更新：**2026-07-29 晚**（G10 bridge · Limitless SAMPLE · fill loop）
...
| **F2 價史補洞** | G10 ledger→日價 | **`g10_kline_price_bridge --write`** | [FILL_LOOP_LESSONS](FILL_LOOP_LESSONS.md) |
| **G 出街** | generation | `canonical_public_snapshot` | [§7 出街](#7-出街-snapshot)（唔使逆向） |
| **H 前端** | 展示 | `apps/web` | [§8 前端](#8-前端)（唔使逆向） |
| **I 填庫 loop** | 量 gap→harvest | `scripts/db_fill_until_green.py` | [FILL_LOOP_LESSONS](FILL_LOOP_LESSONS.md) |

**70+ 個其他 pipelines** = 歷史／G10／診斷 → 唔入日更主線，見 STATE §G Legacy。  
**sparse／SAMPLE／30d 假 0%** 捷徑一律見 [FILL_LOOP_LESSONS.md](FILL_LOOP_LESSONS.md)。
