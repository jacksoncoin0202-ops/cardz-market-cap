...
| #52 | **CSP 擋住 Cloudflare Analytics beacon**：live HTML 有 `cloudflareinsights.com/beacon.min.js`，但同一個 response 個 CSP `script-src 'self' 'unsafe-inline'` / `connect-src 'self'` 兩邊都冇佢。`apps/web/next.config.ts:17,22`。**而家零 analytics** → **CSP 已解封 729af269（2026-08-17）**：`script-src` 加 `https://static.cloudflareinsights.com`、`connect-src` 加 `https://cloudflareinsights.com`。⚠ 呢個只證到「CSP 唔再擋」，**未證到 analytics 有數** —— 出街後要驗：`curl -s https://app.cardzmarketcap.com/ | grep -o "static.cloudflareinsights.com[^"]*"` 有 beacon URL，同埋 Cloudflare Web Analytics dashboard 15 分鐘內見到 pageview > 0。未驗之前唔准講「analytics 修好咗」 |
