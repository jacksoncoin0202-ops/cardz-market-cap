...
> 最後更新：**2026-07-29**（階段定調：本機揾料齊庫；日更之後先做）  
> 數字會漂 → 以下面 `status` 命令即場輸出為準；改完覆蓋要更新 §A／§D。
...
| 優先 | 事項 | 狀態 |
|---:|---|---|
| 1 | **FE Live 100%** | ✅ FE_SET **229** 素材齊 · **1d/7d/30d changePct null=0** · SAMPLE re-scan **0** · 見 [FE_LIVE_100.md](docs/FE_LIVE_100.md) |
| 2 | 價／市值（池） | any_price **940**/940 · snk_price **223** · ebay **137** · tpl **807** |
| 3 | 圖（池） | asset/ptr **940**/940 · disk webp **940** · FE SAMPLE 已換 · **SNK 原圖流水** pilot 93024→v1698 ✅；plan [`docs/SNK_IMAGE_PIPELINE.md`](docs/SNK_IMAGE_PIPELINE.md) · `pipelines/snk_image_ingest.py` |
| 4 | 成交（池） | sale_any **404** · sale_30d **381** · POST_511 harvest 504 bound + ingest 4363 筆；見 [`temp/agent-swarm-20260729/POST_511_kline_trades.md`](temp/agent-swarm-20260729/POST_511_kline_trades.md) |
| 5 | 全池 identity | snk_id **511** · ebay_id **93** · POST_S1S2：merge harvest +2255 · S11 +120 · S8 +1 · match-snk +15 · clean −37 · 報告同上 |
| 6 | 價史 sparse（±5d@30d） | 剩 **2**（ST10-006、Greninja Star——源窗真無 bar） |
| 7 | productionEligible | 仍 False（ranked≠published）；**唔擋 soft live** |
...
| — | **填庫 loop／逆向捷徑** | [FILL_LOOP_LESSONS.md](docs/FILL_LOOP_LESSONS.md) · `scripts/db_fill_until_green.py` |

**用戶優先**：價 → 圖 → 成交／流動性 → FE 成套 → snapshot。  
**PM 填庫**：量 gap → `g10_kline_price_bridge` → SNK bind/harvest/ingest → TPL → Limitless 圖 → rebuild；試盡真源，**唔 invent**。

### 階段定調（2026-07-29 · 用戶）

| 而家 | 之後（未做、唔好跳） |
|---|---|
| **本機 session 揾料**（價／成交／圖／identity 齊缺口） | 料上板／上雲後，**腳本每日自動走期更新** |
| 維護全量池（而家 **940**，遲啲會脹） | 有 registry／preferred 源後先日更；**未齊庫唔開日更**（R8） |
| 目標：先齊料、能入庫、能 mark 成功路徑 | 日更／部署自動化 **用戶話先做** |

一句：**而家只專心本機揾料齊；日更自動化遲講。**

**價史深度（用戶 2026-07-29）**：**近 30 日有 bar 就夠 score／升跌**；更長史 welcome（一掣 harvest 就拉），唔使為「全歷史」卡住填庫。KPI 以 any_price + 30d 可用為主。

**入庫前驗證（用戶 2026-07-29）**：搵料易；**入庫前驗 = 正式工序**。有腳本用腳本（`verify_pair` / SAMPLE / PSA10 閘）；殘渣先 AI 半自動。清單：[`docs/INGEST_VERIFY_GATE.md`](docs/INGEST_VERIFY_GATE.md)。報進度分：**Harvest｜DB 寫入｜驗證綠**。

**圖 QC（用戶 2026-07-29 糾正）**：**全部 TCG 都要 QC**。SAMPLE **水印**主要出喺 **OP／海賊王**——唔等於 PTCG 免驗；PTCG 仍要身份／錯卡／缺圖。

**循環（用戶）**：**QC 完再揾卡**。用 reject／accept pattern 分辨可靠 source；**SNK 可用但必先 QC**（命中率極不穩）。見 [`docs/SOURCE_QC_PATTERNS.md`](docs/SOURCE_QC_PATTERNS.md)。

**Identity 搜尋基本功（用戶 2026-07-29 硬糾正 · 必先記得）：** 擴 SNK／eBay id **唔係**淨跑腳本。每張卡必做人類 search ladder——**變種** × **日文名+英文名** × **編號多寫法**（大小寫／`-`／空白／去零／分母）。權威：[`docs/IDENTITY_HUMAN_SEARCH_PLAYBOOK.md`](docs/IDENTITY_HUMAN_SEARCH_PLAYBOOK.md)。派 agent 前無講呢三樣 = 未開工。

**三層方法論（用戶確認突破 · 必記）：**  
1. **自動化** — browser／過 CF／換 cookie，打通 research 管線  
2. **搵存量** — 已有 harvest 上 exact 變種＋query ladder  
3. **搵增量** — discover 新 apparel → harvest → match → trades  
有增長先加劑量（見 `temp/agent-swarm-20260729/SCALE_WHAT_WORKS.md`）。

**硬目標（用戶 2026-07-29）：DB 內容 100% 全齊。** 窮盡歷史可行方法；本機工廠。權威清單：[`docs/DB_100_EXHAUST.md`](docs/DB_100_EXHAUST.md) · `scripts/db100_exhaust_pass.py`。

**揾價 Skill（2026-07-29）**：`~/.agents/skills/cardz-psa10-price/`（Grok/Claude 已鏡像）。揾卡 skill **等完成後**先寫。之後可對比 ClassOS 搵價插件。
...
> 量度：`qualified_pool_operator status` · **2026-07-28T20:40Z**（UTC）· flood SNK wave 後

| 維度 | 約數 | 解讀 |
|---|---:|---|
| watchlist | **940** | 會再升 |
| 有 PSA10 價 | **940** | TPL 807 / SNK 223 / eBay 137 |
| TPL map / harvest 檔 | **832 / 810** | map rows 932 |
| 有圖 asset / ptr | **873 / 873** | OP Limitless 已批量；SAMPLE FE 已清 |
| SNK id | **143** | semi_auto verify +14、clean −1；淨 −1 vs wave 前 144 |
| eBay id | **90** | 續擴 identity |
| sale 1d / 7d / 21d / 30d / 90d / any | **64 / 206 / 264 / 265 / 271 / 273** | flood trades 2489 + G10 +43 |
| 無價 | **0** | any_price 滿池 |
| FE changePct null 1d/7d/30d | **0 / 0 / 0** | heatmap／表同一欄 |
| FE SAMPLE hits（scan_v2） | **0** | 見 temp/sample_image_fix_report.md |
| SNK flood report | — | [temp/flood_snk_sales_wave_report.md](temp/flood_snk_sales_wave_report.md) |
...
| 流動性歷史 | **G10 sales_cache 本地** | 2–10 分 | `g10_sales_cache_ingest --platforms snkrdunk`（**只寫成交唔寫價**） |
| 價史補洞 | **G10 K 線 bridge** | &lt;1 分 | **`g10_kline_price_bridge.py --write`**（ledger→price_observation prio 300） |
| US 成交 | PC→eBay | 較慢 | `pricecharting_ebay_export`；eBay 直爬 sold **PX 擋** |
| 價尾巴 | TCGFish PSA10 | 中 | `us_price_fallback` / fill_no_price |
| OP 圖（無 SAMPLE） | **Limitless `_EN.webp` 優先** | 秒～分 | `op_limitless_images.py`；P 缺圖 → G10 SNK CDN；**唔用** TCGplayer SAMPLE |
