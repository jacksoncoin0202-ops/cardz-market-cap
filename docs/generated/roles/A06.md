...
3. Read this `A06` pack only; do not read the other eleven role packs.
4. Run `python -X utf8 scripts/backend.py explain A06`.
...
- Stage(s): `A06`.
- Mission: Close exact current price, history, anchors and outlier decisions under approved source policy.
- Architecture nodes: `collector.snk-exact`, `database.canonical`.

### Read scope

- `pipelines/snk_market_data.py`
- `pipelines/snkrdunk_bulk.py`
- `market_price_observation`

### Required documents — demand-load only

- [`snk-manual`](../../SNKRDUNK_API_MANUAL.md) — first run `backend.py explain document:snk-manual`; load only when the current decision needs it.
...
- `price-domain` (`exclusive`)
  - `pipelines/snk_market_data.py`
  - `pipelines/snkrdunk_bulk.py`
  - `tests/test_snkrdunk_bulk_refill.py`
...
- `live-market-research`
  - Load when: the exact work item explicitly authorizes live price or source research.
  - Candidate adapters: `cardz-psa10-price`, `ptcg-price-research`.
  - If absent: report a capability gap; never replace live research with stale prose.
  - Output boundary: `volatile_material_with_provenance`.
...
- all price cells reach a terminal state.
- source identity and price timestamps are exact.
