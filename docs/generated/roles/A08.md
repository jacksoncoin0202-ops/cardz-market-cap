...
3. Read this `A08` pack only; do not read the other eleven role packs.
4. Run `python -X utf8 scripts/backend.py explain A08`.
...
- Stage(s): `A08`.
- Mission: Close semantic card identity, SAMPLE, rights, hash and derivative decisions.
- Architecture nodes: `review.media`, `validator.public-qc`.

### Read scope

- `pipelines/verify_images.py`
- `manifests`
- `apps/web/public/market-assets`

### Required documents — demand-load only

- [`security-policy`](../../SECURITY.md) — first run `backend.py explain document:security-policy`; load only when the current decision needs it.
...
- `image-media-domain` (`exclusive`)
  - `pipelines/verify_images.py`
  - `tests/test_public_snapshot_qc.py`
...
- `media-processing`
  - Load when: the exact A08 work item creates or transforms media rather than auditing identity and rights.
  - Candidate adapters: `media-use`.
  - If absent: report a capability gap; do not substitute unverified media.
  - Output boundary: `private_candidate_before_qc`.
...
- every decision binds the approved image policy.
- public candidate has zero SAMPLE and cross-card images.
