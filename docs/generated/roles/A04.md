# A04 — Script Lifecycle and Call Graph
...
3. Read this `A04` pack only; do not read the other eleven role packs.
4. Run `python -X utf8 scripts/backend.py explain A04`.
...
- Stage(s): `A04`.
- Mission: Classify every executable, caller, writer surface, scheduler edge and lifecycle.
- Architecture nodes: `index.codegraph`, `orchestrator.daily`.
...
- [`tool-registry-view`](../TOOL_REGISTRY.md) — first run `backend.py explain document:tool-registry-view`; load only when the current decision needs it.
...
- `script-inventory-control` (`exclusive`)
  - `pipelines/script_inventory.py`
...
- `architecture-visualization`
  - Load when: the assigned work item explicitly requires a control-plane or dependency diagram.
  - Candidate adapters: `architecture-diagram`.
  - If absent: return the validated registry view without installing or substituting a plugin.
  - Output boundary: `generated_view_only`.
...
- unclassified executables equal zero.
- each protected writer has exactly one controller.

## Handoff

- Next role(s): `A05`, `A06`, `A07`, `A08`, `A09`, `A11`, `A12`.
