# A11 — Bootstrap Control and Runtime
...
3. Read this `A11` pack only; do not read the other eleven role packs.
4. Run `python -X utf8 scripts/backend.py explain A11`.
...
- Layer: `L1_L3`.
- Mode: `SERIAL_PHASES`.
- Stage(s): `A11.bootstrap`, `A11.control`, `A11.runtime`.
- Mission: Run baseline bootstrap, control CLI integration, then full/delta orchestration, locks, receipts and retry.
- Architecture nodes: `orchestrator.daily`, `state.last-good`, `status.machine`.

### Read scope

- `scripts/backend.py`
- `pipelines/run_daily.py`
- `pipelines/run_receipts.py`
- `deploy`

### Required documents — demand-load only

- [`backend-runbook`](../../RUNBOOK.md) — first run `backend.py explain document:backend-runbook`; load only when the current decision needs it.

### Owned write claims

- `runtime-control` (`exclusive_serial_phases`)
  - `scripts/backend.py`
  - `scripts/pc_full900_supervisor.ps1`
  - `pipelines/run_daily.py`
  - `pipelines/run_receipts.py`
  - `pipelines/failure_ledger.py`
  - `pipelines/pc_full_serial_driver.py`
  - `tests/test_run_daily_orchestration.py`
  - `tests/test_failure_ledger.py`
  - `tests/test_failure_integrations.py`
...
- `collector_business_logic_change`
...
- baseline is frozen before domain work.
- full and incremental use one engine.
- retry resumes and same input is a no-op.
