...
3. Read this `A12` pack only; do not read the other eleven role packs.
4. Run `python -X utf8 scripts/backend.py explain A12`.
...
- Stage(s): `A12`.
- Mission: Attack contracts, replay, privacy, writer containment and release claims independently.
- Architecture nodes: `validator.canonical-db-qc`, `validator.public-qc`, `status.machine`.

### Read scope

- `tests`
- `docs/generated`
- `data/runtime/private-reports`

### Required documents — demand-load only

- [`document-authority-view`](../DOCUMENT_AUTHORITY.md) — first run `backend.py explain document:document-authority-view`; load only when the current decision needs it.
...
- `red-team-tests` (`read_only_production`)
  - `tests/acceptance/**`
  - `data/runtime/private-reports/red-team/**`
...
- verdict cites exact commands, hashes and raw test output.
