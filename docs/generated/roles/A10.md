# A10 — Derived Metrics and Ranking
...
3. Read this `A10` pack only; do not read the other eleven role packs.
4. Run `python -X utf8 scripts/backend.py explain A10`.
...
- Stage(s): `A10`.
- Mission: Own windows, market cap, ranking, view cardinality and truthful UI semantics.
- Architecture nodes: `derivation.market`, `ranking.scopes`, `export.views`.

### Read scope

- `pipelines/ranking_derivation.py`
- `pipelines/market_alerts.py`
...
- `derived-ranking-domain` (`exclusive`)
  - `pipelines/ranking_derivation.py`
  - `pipelines/market_alerts.py`
  - `tests/test_ranking_derivation.py`
...
- one formula path is used.
- rankings are contiguous and projections preserve truthful coverage.
