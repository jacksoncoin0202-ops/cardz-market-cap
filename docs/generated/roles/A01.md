...
3. Read this `A01` pack only; do not read the other eleven role packs.
4. Run `python -X utf8 scripts/backend.py explain A01`.
...
- Stage(s): `A01`.
- Mission: Map every page, API, selector, filter, sort and cardinality to stable public fields.
- Architecture nodes: `export.views`, `export.sanitized-snapshot`.
...
- `frontend-design`
  - Load when: only after MAIN supplies an accepted versioned frontend target-state contract; consumer census alone does not trigger it.
  - Candidate adapters: `design-taste-frontend`.
  - If absent: return CONTRACT_GAP to MAIN; do not infer target behavior from quarantined design notes.
  - Output boundary: `contract_gap_candidate_only`.
...
- every consumer and requested cardinality is mapped.
- no frontend component defines backend authority.

## Handoff

- Next role(s): `A02`, `A09`, `A10`, `A12`.
