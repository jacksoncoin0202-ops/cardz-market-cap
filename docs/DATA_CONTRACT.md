...
This contract defines the only route from private G10, GemRate, and standby observations to the public CARDZ Market Cap product. Public UI code consumes only a sanitized, versioned snapshot. Direct reads from `grade10-scraper`, `cardz-platform`, private landing objects, replay SQLite, or provider APIs are release-blocking errors.
...
The initial full generation is imported once. A daily collector may refetch all tracked cards, but the importer appends only new observations. Every run records source hashes, effective and fetched timestamps, row counts, accepted/quarantined/rejected counts, and completion state. Raw objects may contain provider IDs and URLs and therefore never enter public paths or browser bundles.

### Production canonical history

JLP MySQL 5.7 is the sole writable production authority. Existing canonical printing, variant, price history, population history, and market-cap tables remain authoritative. Additive market tables record ingest runs/checkpoints, source observations, index/constituent snapshots, grader populations, tracked-sales aggregates, image versions, and QC.
...
```text
Private G10 full + daily runs
          + GemRate identity/population corrections
          + private standby source when explicitly enabled
                              ↓
                    private landing evidence
                              ↓
             exact identity, freshness, sales, image QC
                              ↓
                     JLP canonical history
                              ↓
...
- Initial upstream 30d change may be published only after validation. New 1d/7d values remain accumulating until canonical daily anchors exist.
...
## Ranking and grader markets

The combined V1 Top 100 uses one comparable formula:
...
Eligible cards are sorted by unrounded market cap. Public Top 100 contains exactly ranks 1–100; up to the next 400 eligible cards become the contiguous watchlist at ranks 101–500. The watchlist is never padded with ineligible cards. If fewer than 100 pass, production publication fails.
...
The V1 demo grader routes currently project grader population fields from the PSA-eligible public subset. They are not a complete canonical grader supply universe and must not be described as an all-market BGS, CGC, or SGC ranking. Production remains blocked by `grader_supply_universe_pending` until JLP exports an independently gated supply universe for each grader.
...
Every Top 100 printing has independent English, Traditional Chinese, Simplified Chinese, and Japanese name, set, and market-story content. Production cannot silently fall back to English. Chinese copy uses written language.

USD is the stored base. HKD, CNY, GBP, and TWD use the generation's freshness-checked FX observation; conversion never mutates base metrics. Locale, currency, and selected window persist in canonical public routes.

## Publication sequence

1. Acquire a private full or daily run and finalize its manifest.
2. Validate completeness and import it idempotently into JLP canonical tables.
