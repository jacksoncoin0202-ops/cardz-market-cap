...
| **kiosk 特效層** | kiosk 呼吸感／掃光／自動聚光巡遊／邊框脈動／入場 burst／底噪（owner 2026-08-18：「超誇張少少嘅呼吸感」＋「你到去邊張要飛去中間比大家睇到先得」＋「#3 太細，呢個係排行嚟」）。原型同實測喺 `temp/fe05/kiosk-fx/`（`kiosk-fx.css` / `kiosk-fx.js` / `kiosk-fx-notes.md` / `perf.json` / `throttle-ladder.json` / `reduce-proof.json`），**未入 `apps/web/`**。已知數：60Hz 之下 44 個 run median 全部 59.9fps、CLS 全部 0，所以未節流數字冇資訊量；**要睇 CPU 節流梯** —— 守得住 55fps 嘅最大倍數 baseline 5× / sweep 4× / breathe **2×**，`recalcStyleDuration` 10 秒 delta breathe **255ms**（tour 27 / edge 33 / sweep 48 / noise 0）。「細格唔派呼吸」試過：動畫由 99 條減到 74 條，3× 節流一樣 30.1fps ⇒ 樽頸係逐幀掃成塊板嗰個 style pass，唔係條數，**已 revert**。reduce sibling 17/17 PASS。未做：FLIP 飛中間 + hero rank 數字 + 唔截斷卡名落 `heatmap.tsx` | TODO |
