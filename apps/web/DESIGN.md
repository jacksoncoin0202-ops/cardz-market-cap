...
| **fe05(rank-size)** | 榜單每頁數量掣 100 / 200 / 500 + 畫面行數封頂（owner 2026-08-18：「每頁展示嘅項目數量，俾人去揀 100、200 或者 500 個，唔好有一千個喇⋯⋯有陣時我碌下碌下落到去，原來 show 到成 800 個項目，部機就會 lag 機，我要 F5 refresh 一次先可以更新返」）。**「最多揀到嘅每頁數量」同「畫面最多幾多行」由今日起係同一個數**：`RANKING_ROW_CAP = Math.max(...RANKING_PAGE_SIZES)` —— **唔准寫死**，寫死嘅話下次有人加個 800 落選項，cap 就會靜靜變咗「揀到但顯示唔到」。`RANKING_PAGE_SIZES` 由 `[100,200,300,500,1000]` 收做 `[100,200,500]`。**300 唔准直接踢 404**：榜尾嗰排數量掣一直係真 `<a href="?size=300">`，出過街即係可能已經俾人 bookmark／爬蟲收咗 —— 開 `LEGACY_PAGE_SIZES = [300]`，parser 照收、**任何 UI 都唔再出**（呢個 list 只會縮唔會長）。**順手修一粒會俾 cap 揭出嚟嘅舊 bug**：`nextHref` 係 server 算 `page + 1`，但接咗 4 版落去之後真正「下一版」係 `page + 5` —— 撞到 cap 之後 `›` 就係唯一逃生口，指返已經睇過嗰版就等於死路。function 過唔到 server→client 界，所以改成派 `pageHrefs: string[]`（**成個榜每版一條**，size 100 × 1604 張 = 17 條短字串，payload 忽略得），client `nextHref = pageHrefs[lastLoadedPage]`。**舊 cap 兩個洞**：`AUTO_APPEND_ROW_CAP = 800` 只夾自動接、撳「展示更多」照過；而且判嘅係**接之前**（`loadedRows < cap`）—— size 500 之下 500 < 800 成立，接完就 1000。而家一律 `loadedRows + pageSize <= RANKING_ROW_CAP`，自動同手動同一條。撞頂之後個掣會消失，所以喺原本嗰條 `aria-live` status 出一句本地化解釋（`labels.rowCapReached`，五語言，`{count}` 喺 server 填死，唔想連 `fillTemplate` 都拖埋落 client）。**擺位**：桌面榜頂新開一行 `.ranking-filter-row`（`flex-wrap`，語言列 + 數量列共用 `.lang-filter` 個樣；margin 掛喺 row 唔掛喺 `.lang-filter`，否則 wrap 之後多咗一段空隙）；**手機 ≤680 同語言列一模一樣搬入 `SortFilterSheet`**（新「每頁」段）—— 唔另開新 pattern。`/box` 唔行 `?size=` 分頁，所以 `pageSizePicker={false}`、`pageSize` 只填預設頂住 type；搜尋模式 `pager` 係 `null`（行 `?show=`）亦唔出掣，出個揀唔郁嘅掣就係呃人。換數量**一定要一齊 `page: 1`**（喺第 5 版揀 500 唔重設就變 `?page=5&size=500` = #2001 起，1604 張直接 404）；`use-market-settings.ts` 讀 **live URL** 唔讀 props，換完即刻著返個掣，唔使等 RSC 行完。**驗收**（`temp/fe05/rank-cap/{verify-rank-cap,shots}.mjs`，dev :3901 有頭 Chromium 1440×900）：起錶 100 行、榜頂同 pager 兩處都係 `[100,200,500]`；碌 12 次 → 400 → **500 之後永遠 500**、掣收起、`next=/?page=6`（唔係 `page=2`）、status 出咗解釋；1440 語言@43+225／數量@276+221 **同一行 y162**，1024 同一行 y150，768 自動疊兩行，三個闊度 `scrollWidth` 都冇爆；撳 `›` → `/?page=6`、第一行 **#501**、榜頂落喺 viewport 頂（−18…6px，唔係 `scrollY 0`）；390 榜頂 `.ranking-filter-row` **唔存在**、排序 sheet 三段 `[Sort by / Language / Per page]`（截圖自己 Read 過）。**兩條 fault 逐條種返證紅**：A `RANKING_PAGE_SIZES` 加返 `1000` → 數量掣❌ + 見過 **1000 行**❌（順帶證實 cap 真係跟 `Math.max` 走，唔係另一個寫死嘅數）；B `RANKING_ROW_CAP` 改死 `2000` → 見過 **1100 行**❌ 而數量掣仍然✅（即係三條判詞真係各自獨立）；兩個 fault exit=1，還原後重跑 exit=0。⚠️ **試過用 `page.addInitScript` 注一粒假 `1000` 掣入 `.page-size-filter`：唔得，React 一 re-render 就掃走個 node，個閘照樣綠** —— 「注唔入去所以睇落冇事」正正就係假閘，fault 一定要落 source 種。**驗證腳本嘅 `EXPECTED_SIZES` / `ROW_CAP` 係故意寫死唔 import 實作**（import 咗 = 兩邊一齊錯都驗唔到）。**誠實講一句**：size **200 之下自動接停喺 400 行唔係 500**（再接一整版就 600，爆 cap）—— 設計如此，唔係壞咗；要睇 401–600 撳 `›`。**未收殮**：cap 只擋 append，冇擋「一開就 `?size=500`」嗰 500 行本身（owner 講 800 先 lag，500 係佢自己揀嘅上限）。**點樣反轉**：`git revert` 呢粒 —— 數量掣退返 100/200/300/500/1000、cap 退返 800（連同兩個舊洞）、`›` 退返指 `page+1`、手機 sheet 少一段 ⚠️ **同日已被下一行 `fe05(rank-topbar)` 改寫**：`RANKING_ROW_CAP`（跟 `Math.max(...RANKING_PAGE_SIZES)` 走）已經唔存在，數量掣亦放返 300／1000 —— 呢一行嘅 cap 講法只當歷史睇 | ✅ 已落（cap 部分已被取代） |
| **fe05(rank-topbar)** | 300／1000 放返落選項 + 榜頂講返實數 + 「返回頂部」（owner 2026-08-18 同日第二輪：「咁你咪加返 300 落去囉，鍾意 1000 加埋都得，唔大問題」／「我向下碌，譬如話去到五百嗰個時候，跟住之後睇返頂頂嗰個頁面嘅都冇話係五百」／「碌到最低⋯⋯即係喺呢一個揀選幾多百幾多百第二頁嗰度，有一個掣撳一撳就返回至頂」）。**上一行嗰條 `RANKING_ROW_CAP = Math.max(...RANKING_PAGE_SIZES)` 喺呢一刻會反咬**：一加 `1000` 落選項，「自動接落去」個上限就靜靜由 500 跳去 1000，owner 原本投訴嗰個 lag 直接返晒嚟，而且冇一處睇得出。所以拆做兩件事 —— `AUTO_APPEND_ROW_CAP = 500`（**累積**上限；owner 投訴嘅係「我冇要求過但佢自己接到 800」）同 `rankingRowCap(pageSize) = Math.max(pageSize, AUTO_APPEND_ROW_CAP)`（主動撳「1000」就係佢自己要一版 1000 行，唔准收埋一半當冇事發生），`pagination.ts` 檔入面明寫**唔准**寫返 `Math.max(...RANKING_PAGE_SIZES)`。實際效果（1604 張）：size 100 接到 500、200 接到 **400**、300／500／1000 一版到底唔再接（再接一整版就爆），`›` 仍然係逃生口。`RANKING_PAGE_SIZES` 回 `[100,200,300,500,1000]`、`LEGACY_PAGE_SIZES` 清空但**機制留住**（下次再砍數量要有位擺，唔可以順手踢做 404）。**「頂頂冇話係五百」係量完先修**（`temp/fe05/rank-cap/probe-top-state.mjs`）：H2 **一直都有**跟住行數走（接完真係寫住 `Top 500 by market cap`），對唔上嘅係每頁數量掣仍然 highlight `100` —— 佢真係 100/版，所以**唔准**改去 highlight「500」（「每頁」同「而家幾多行」係兩件事，撈埋換版之後即刻對唔上）。改為喺同一組膠囊尾加一個獨立讀數 `.page-size-loaded`（`labels.rowsShown` 五語言，client 用 `visibleCards.length` 填），中間一條 `border-inline-start` 分開「你揀嘅設定」同「而家實際幾多行」。**「返回頂部」**擺喺榜尾 pager 同一行（owner 指定嗰個位），係真 `<a href="#market-ranking">`（冇 JS 都用得，同 pager 其餘 link 一致），有 JS 就 `preventDefault()` + `scrollIntoView({ block: "start" })` —— 唔想每撳一次就 push 個 hash 落 history（撳返上一頁會彈返落底）。**返嘅係榜頂唔係文件頂**：owner 話明返上去就係為咗睇／改嗰組掣，一嘢摙返上 hero 就又要再碌落嚟。≤680 CSS 收起個字剩支箭嘴（`aria-label` 照留），因為 390 嗰行已經有 5 個數量掣 + `‹ ›` + 「展示更多」。**量度陷阱**：`.rankings-section` 有 `scroll-margin-top: var(--header-height)`，所以正確落點**唔係 0**、係 header 高度（桌面 68、手機 92、某幾個斷點 116）—— 判詞要讀返個 token 嚟比，硬寫 `68` 就係一條喺手機亂嗌嘅假閘。**驗收**（`verify-rank-cap.mjs` 七條判詞，dev :3901 有頭 Chromium 1440×900、`ROUNDS=9`）：數量掣兩處都係 `[100,200,300,500,1000]`；碌極唔過 500；真係去到 500；`next=/?page=6`；掣收起兼出咗解釋；榜頂讀數 起錶 100 → 接完 **500**；撳「返回頂部」→ `top 68px` vs `header 68px`、URL **冇多 hash**、行數仍然 500 —— 七條全綠 exit=0。**四條 fault 逐條落 source 種返證紅**：A `rankingRowCap()` 改返 `Math.max(...RANKING_PAGE_SIZES)` → 見過 **1000 行**❌ + 冇停喺 500❌（連帶 `next` 變 `page=11`、讀數變 1000 一齊紅），而數量掣仍然✅ —— 正正示範「加咗 1000 落選項就靜靜拉高咗 cap」呢個 regression 真係有人守；B 剝走 `300` → 數量掣❌ **其餘六條全綠**（反過來證實 cap 唔跟 list 走）；C 拆走 `.page-size-loaded` → 讀數❌（`起錶 null／接完 null`）**其餘全綠**；D 拆走 `scrollIntoView` 嗰行 → 返頂❌（`top -44433px`）**其餘全綠**。**版面自己 Read 過**（`shots.mjs`）：1440 榜頂一行擺得晒（語言@43+225／數量@276+**415**，同一行 y230，`scrollWidth` 1425 冇爆）、1024 同一行、768 自動疊兩行；1440 pager **一行**（`↑ Back to top` 右邊 1382 < 1425）、390 pager wrap **三行**（箭嘴掣右邊 213 < 390、`.ranking-to-top-text` computed `display:none`）、390 排序 sheet「每頁」段五粒掣一行擺得晒。⚠️ **fault drill 自動化喺呢部機唔穩**：連環開有頭 chromium 會積死進程（見過 8 個 `ms-playwright\…\chrome.exe`），之後新開 browser 一 scroll 就 `Target crashed`（得 100 行都死，唔關 DOM 大細事）；`Stop-Process` 殺唔到，要 `Get-CimInstance Win32_Process` + `Terminate` —— 所以四條 fault 最後係**逐條手動**跑。**另一個坑**：drill 一 crash 就唔會寫新 `report.json`，讀返上一次嗰份 = 攞乾淨跑嘅綠燈當 fault 結果，所以每次跑之前一定要 `rm` 咗佢先（已寫入 `fault-drill.mjs`）。**閘**：`tsc` 0；lint 0 error / 8 warning（基線）；`npm test` 54/56（紅嗰 2 條係 python data-lane 搵唔到 `data/runtime/config/backend.env`，呢個 FE worktree 本來就冇）。**未收殮**：手機 ≤680 冇 `.page-size-loaded` 讀數（榜頂控件行喺 390 根本唔 render，H2「Top 500 by market cap」係嗰邊唯一讀數）。**點樣反轉**：`git revert` 呢粒 —— 數量掣退返 `[100,200,500]`、cap 退返「跟 list 最大值走」、榜頂讀數同「返回頂部」一齊消失 | ✅ 已落 |
| **fe05(kiosk-enter-cut)** | kiosk 入場動畫**成個剷咗**，撳落全屏第一幀就係最終狀態（owner 2026-08-21，同一件事投訴第二次：「一入到去都係走位⋯⋯跟住我移一移位或者做啲操作，跟住又正常翻」；owner 拍板「第一眼定住好似最舒服」）。**唔係排版錯**：逐幀量 tile 嘅 layout box，由 0ms 起四邊都係 2px，`imgOutOfTile` 全程 0/100 —— treemap / ResizeObserver / snapCardBox / srcset 全部洗脫。真身係入場 burst 自己 scale `.heatmap-tile` 個**盒**，而格與格之間條白隙係釘死喺 device px 格上面（`lib/pixel-snap.ts` 分 `p=floor(g/2)` / `q=g-p`，相鄰兩格共用同一條 rounded 邊）；盒一 scale，每格就向外食走自己嗰條隙、而且各自 `animation-delay` 唔同步 → 成版睇落「啲卡走晒位」。同一份 CSS 早咗 380 行喺 `kiosk-breathe` 個註**已經寫低咗呢條規矩**（scale 嘅要係入面嗰張 `.tile-card`，唔係 tile 盒），burst 就係違返自己份文檔。**實測**（live app.cardzmarketcap.com，headed Chromium，逐幀量相鄰 tile 之間嘅隙）：入 kiosk 前全版一律 **5.00 device px**（dpr 1.5）／3.00（dpr 1）；Enter 後 185ms → **8.3…16.93px**，100 格全部凍喺 `scale(0.94)`（`backwards` 填住 delay 窗）；221…455ms → 9.11…17.69px 仲係 0.94；之後 `--ease-spring` 過衝到 1.0059 先埋位；最後一幀花嘢 **851ms**，105 幀入面 **30 幀**係花嘅。**dpr 1 一樣中招**（3px → 2.2…9.85px）—— 唔係 scaling 問題。「移一移位就正常返」唔係郁鼠標嘅功勞，係 burst 自己播完；arm effect deps 係 `[width, height]`，全屏 transition 期間 ResizeObserver 派多過一次 size ⇒ **重播**，所以真機感覺長過 0.85s。**改法 = 剷，唔係調細**（0.94 已經係第二次縮細嘅版本，一樣量到走位）：CSS 拆 `@keyframes kiosk-burst` + `[data-kiosk-enter]` rule + reduced-motion sibling + `--kiosk-burst-delay/-dur` 兩個 token；tsx 拆 arm/disarm effect + `BURST_MAX_DELAY/DUR/TAIL`；`heatmap.tsx` 拆 `fxRadius()` + `--fx-r`（零 call site 剩），`--fx-i` 留低（`kiosk-breathe` 要）。`readMs` 留低（fly 時鐘仲用）。**驗**：`temp/fe05/kiosk-fx-land/measure-enter-gaps.mjs` 連續逐幀量隙 —— clean **0/89 走位幀**，全程鎖死 5.00，帶 transform 嘅格 **0**，`data-kiosk-enter` 冇出現過；`SEED=1` 種返舊 burst 落 page（唔改 source）**24/91 紅、3.34…12.95px、100 格有 transform** ⇒ 條判斷真係會 fire（規矩 9）。`tsc --noEmit` 綠；C8 `temp/fe05/review-webfont/verify-kiosk.mjs` **PASS**（burst 走咗，佢同 fillScan/unionDH 鬥時間嗰 270ms 餘量問題一齊消失）。⚠️ `temp/fe05/kiosk-fx-land/verify-kiosk-fx{,-css}.mjs` 入面 ②④ 兩組 check 係斷言 burst **存在**嘅，特效剷咗之後佢哋按定義會紅——**唔准為咗綠而改返個閘**，佢哋係過時，唔係發現咗問題。 |
| **fe05(tile-transform-gate)** | 上面嗰粒嘅**收殮**：量到 bug 嗰條判斷升做 tracked release gate，新 `scripts/test-fe-heatmap-tile-transform.mjs`。**點解一定要做**：條規矩本身自 `kiosk-breathe` 個註（2026-08-18）就寫低咗，但淨係寫喺註釋度 ⇒ 同一個錯**犯咗兩次**（08-18 縮細、08-21 剷走）；而證實到佢嘅 `temp/fe05/kiosk-fx-land/measure-enter-gaps.mjs` 喺 `temp/`（gitignore），照 repo 自己嗰條「**任何『有檢查但零 call site』都當冇檢查**」= 對第三次翻叮**零防護**。**兩層**（跟 `scripts/test-fe-detail-rail.mjs` 個先例：static 為主、`--live` opt-in），因為 playwright 唔係 repo dep、`npm test` 路徑上冇 dev server —— 靜態嗰層先係日日行嘅嗰層。**靜態（默認，零 infra）**：T1 brace-scan CSS（連 at-rule stack），逐條 rule 睇佢個 **subject**（selector 最後一個 compound，`sel.split(/\s*[>+~]\s*|\s+/).pop()`）有冇 `heatmap-tile`，再睇佢加唔加到 transform（直接寫 `transform:` 非 `none`，或者 `animation` 個名指去一個掂 transform 嘅 `@keyframes`）—— 中咗就紅，除非喺 allowlist：`SINGLE_TILE`（`[data-kiosk-star]`，得一格飛緊，唔關隙事）／`INTERACTION`（`:active`/`:hover`/`:focus-visible`，用戶自己撩先動）。T1 仲有一條反向閘：**掃到 0 條就當紅**（一條都揀唔中 = 掃描器壞咗，唔係真係乾淨）。T2 剷走咗嘅名唔准返嚟（`@keyframes kiosk-burst`／`data-kiosk-enter`／`--kiosk-burst-delay`／`--kiosk-burst-dur`／`BURST_MAX_DELAY|DUR|TAIL`／`fxRadius`／`--fx-r`）。T3 `--fx-i` **要仲喺度**（tsx 寫、CSS 讀 —— 剷 `--fx-r` 嗰陣好易連佢一齊剷，`kiosk-breathe` 就熄）。T4 `kiosk-breathe` 一定要落喺 `.tile-card` 而每個 keyframe 都要帶返 `translate(-50%, -50%)`。**掃描之前一定要剝註釋**：呢個 repo 嘅 CSS 註釋本身就寫住 `kiosk-burst`、`--fx-r` 呢啲字，唔剝就自己嗌自己。**`--live`（opt-in）**：原本嗰個逐幀量隙搬咗入嚟做 T6，headed Chromium `--force-device-scale-factor=1.5`，入 kiosk 之後每個 rAF 量一次相鄰 tile 之間嘅隙。**負控制係設計嘅一部分，唔係 bonus**：靜態層有 **7 條 in-process probe**（P1 burst 原文／P2 換咗名嘅入場動畫／P3 直接寫 transform／P4 transition 落盒／P5 subject 係 tile 但寫喺 selector 後面 —— 五條**一定要紅**；P6 `scale` 落 `.tile-card`／P7 `transform: none` —— 兩條**一定唔准紅**），T6 就 `addStyleTag` 種返舊 burst 落 page（**唔改 source**）。**實測**：clean = 100 格、基準 **5 device px**、89 幀連續、走位 **0** 幀、帶 transform 嘅盒 **0**；seeded = 走位 **23/89** 幀、隙散到 **3.34…18.71px**、100 格全部有 transform ⇒ 條判斷真係會 fire。**規矩 9 落真 source 種返三次**（唔淨係 in-process）：burst 種返落 `heatmap-kiosk.css` → **5 條紅**（T1 + 四條 T2 名）；`--fx-i` 改名做 `--fx-r` → **2 條紅**（T2-js + T3）；`kiosk-breathe` 搬返上 tile 盒 → **2 條紅**（T1 + T4）；三次都 `git checkout --` 還原，`git status` 空返，gate 回綠。**接線**：`scripts/run_all_tests.py` 本身就 glob `scripts/test-*.mjs`，所以**唔使加任何 wiring**，`npm test` 即刻見到 `PASS scripts/test-fe-heatmap-tile-transform.mjs 0.1s`。**順手發現一條同科債，冇偷偷改**：`globals.css` 嘅 `.heatmap-tile[data-late]` 行 `tile-pop`，`0.9 → 1.02 → 1` scale 嘅一樣係 **tile 個盒** —— 同 burst 同一個病，分別只係範圍細（只係新到嗰幾格、200ms）而且係 owner 2026-08-16 親自要嘅（「一拉就即刻飛出嚟」）。**唔准我自己改 owner 面前嘅視覺**，所以凍結咗做 `KNOWN_DEBT` 唯一一條（連 `why`／`fix` 寫死喺 gate 入面）：今日 **PASS**、每次跑都印出嚟、但**任何新嘅大規模盒 transform 一律紅** = 一條只縮唔漲嘅 ratchet；而且**債還咗唔刪就一樣紅**（`KNOWN_DEBT` 有條目但搵唔返對應 rule ⇒ fail，唔准留住一條指去唔存在嘅 selector 嘅「債」扮有人管住）。寫定嘅修法：`opacity` 留喺 `.heatmap-tile`，`transform: scale(...)` 搬落 `.heatmap-tile[data-late] .tile-card`，keyframe 帶返 `translate(-50%, -50%)` —— **等 owner 拍板先做**。**同一輪剷埋上面嗰粒留低嘅過時 check**（`temp/`，唔入 git）：`verify-kiosk-fx.mjs` 拆咗 ② 嗰半條 `--fx-r`（連 `withR`/`rRange`/`nearR`/`farR` 同 `data-kiosk-enter` 嗰兩處）同成條 ④ → **11 條全綠 PASS**；`verify-kiosk-fx-css.mjs` 拆咗 `--kiosk-burst-delay` token、`burst/fly` 嗰條合併 check（`fly` 嗰半留返做獨立 check，冇放鬆）同成個 ④ 定格 320ms section → **39/39 pass**。**冇改任何 app source**，所以呢粒**唔帶 `[deploy]`**。**誠實報一條唔關我事嘅紅**：`scripts/test-fe-share-file.mjs` B9 由 `1ee5beaa` 起就紅（`git show HEAD~1` 對過，同我無關）—— 佢 grep 一個 backtick template literal 攞 `filenameBase`，但 `heatmap.tsx` 而家行 `heatmapOgFilename({...})` helper ⇒ **條 regex 盲咗**（佢想守嗰個「唔准孖副檔名」實際上仲係啱嘅）。呢個正正又係「有檢查但零 call site」同一個形狀，但佢屬另一條 feature 線兼有另一個 session 改緊，**唔准為咗個數靚而郁佢**（規矩 10），留返俾 owner 決定 | ✅ 已落（`tile-pop` 債未還） |
| **fe05(share-3x4-16x9)** | 分享尺寸補齊兩個：**`portrait` 1080×1440（IG feed／grid 3:4）**同**`widescreen` 1920×1080（16:9）**，owner 貼圖嗰四個面（X／Threads／WhatsApp／IG）連埋呢兩個先算齊，冇人再需要手動 crop。**加法係加，唔係改**：`SHARE_TARGETS` 七行、四個舊 format、舊 alias（`portrait`／`landscape`／`story`／`whatsapp`）、舊檔名比例字全部一個字冇郁。新 alias `3x4` / `ig-portrait` / `grid` → portrait，`16x9` / `widescreen` / `hd` / `youtube` → widescreen；選單兩行喺新嘅 `SHARE_EXTRA_TARGETS`，`SHARE_MENU_TARGETS = [...SHARE_TARGETS, ...SHARE_EXTRA_TARGETS]` 一張表出，i18n 五語齊。**⚠️ `portrait` 呢個 alias 冇搶**：佢由第一日起指住 4:5 嘅 `post`，而條 HERMES 自動鏈（唔喺呢個 repo）真係日日叫 `?format=portrait`；搶咗就係靜靜換咗佢每日出嗰批圖（同 `whatsapp` 嗰單一模一樣）。要 3:4 就明寫 `?format=3x4`；`landscape` 一樣仍然係 1.91:1 `wide`（og:image 靠佢）。**因為個名同 alias 唔同，所以多咗一張 `FORMAT_QUERY_NAME`**（format 名 → `?format=` 寫咩）＋ guard：冇佢嘅話 CLI 一句 `--format portrait` 就靜靜攞返 4:5，照 200、`x-og-format` 仲會講「post」，差 90px 高冇人發現 —— 而家 CLI 砌 URL 前譯一次，會印返 `--format portrait → ?format=3x4`。**三處真嘢**：① `heatmap-og.ts` 新 `FORMAT_RATIO_LABEL`（檔名比例字由一張表出，唔再由 `w/h` 估）—— 估法會將 3:4 叫成 `4x5`、16:9 叫成 `wide`，兩張圖同一個檔名直接互相覆蓋；順手修返 CLI 一路將 square 叫 `4x5`、server 叫 `1x1` 呢個舊 bug。② 熱力圖 OG 新 `CHROME_SCALE`：標題／stamp／legend 跟**畫布闊度**放大（widescreen = 1920/1080 = 1.778），舊四個釘死 **1** 所以出咗街嗰批圖一個 pixel 都冇郁；格仔嗰邊（gap／label／圓角）繼續行 `scale`，treemap 本來就按闊高自己排，**零 per-format 版面分支**。widescreen 1080p 度到板面 **1834×837**（pad 43／header 57／legend 28／boardGap 36），3:4 度到 **1032×1304**（post 同闊但高多 90px＝同樣柱數、格仔高啲）。③ 卡片 OG：`widescreen` 係**橫**但唔係 `wide`，所以九處 `format === "wide"` 全部改 `isWideFormat()`，加 `WIDE_FORMATS` + import-time 橫直 guard；再加 `FORMAT_LAYOUT_SCALE`（widescreen = 1920/1200 = 1.6）—— 1920 闊畫布照 1200 闊版面畫，個圖會細細粒黏喺左上角但照 200；`WideLayout` 收埋 `canvasHeight`，唔係 1080 高畫布下面剩返一條背景。**五個 import-time guard**（本來三個）：橫直分類、format 冇 alias 叫得到（＝死 code）、選單↔alias drift、比例標講大話、`FORMAT_QUERY_NAME` 叫唔返自己。**實測**（dev :3901，`x-og-format` header + sharp 量真 pixel）：`?format=3x4` → **1080×1440**（`x-og-format: portrait`）、4K **2160×2880**（37.8s）；`?format=16x9` / `?format=widescreen` → **1920×1080**、4K **3840×2160**（47.7s）；en／zh-TW／zh-CN 三個語言各行一次，全部 200。**回歸**：`wide` 1200×630、`square` 1080×1080、`post` 1080×1350、`status` 1080×1920 一個都冇郁。⚠️ `?format=portrait` 照舊出 **1080×1350**（post）—— 呢個係設計，唔係漏咗。CLI `--format portrait --base http://localhost:3901` 出 `cardz-heatmap-all-top40-7d-3x4.jpg`（1080×1440，4.3s）。`npm run typecheck` 0、`npm run lint` 0 error（8 個 warning 全部喺我冇掂過嘅檔）、`scripts/test-fe-*.mjs` **19/19 PASS**（`test-fe-share-destinations.mjs` 新增 ⑧ 段，連四個種返 bug 嘅 mutation probe：橫直分類講大話／新 format 冇 alias／檔名比例標講大話／wire 名叫唔返自己，四個都證實會炸）。**點樣反轉**：`git revert` 呢粒 —— 兩個新尺寸消失，舊四個一個字都冇改過所以唔受影響 | ✅ 已落（**未 commit**，等 owner 睇） |
...
唔改 treemap「面積 ∝ 市值」算法（tile 幾何已加 device-px snapping，見 fix-heatmap-align）。顏色算法**已唔係**非目標：owner 2026-09-23 批咗預設 gamma 4→1.5、aMin 0.78→0.45，分享圖 `api/og/heatmap` 同網站共用 `lib/tile-style.ts` `tileStyle`，冇數格加斜紋、同持平（0.0%）分開；2026-09-25 owner 再批：飽和點逐窗口（1D 5%、7D 20%、30D 50%、90D 70%、180D 120%、365D 250%，`WINDOW_CLAMP_SCALE`），升跌幅改用倍數（log）量，+100% 同 −50% 一樣深；唔加 `app/card/loading.tsx`；
