...
export const hubCopy: Record<Locale, HubCopy> = {
  ...
  ja: {
    ...
    common: {
      home: "マーケット",
      breadcrumb: "パンくずリスト",
      method: "算出方法：時価総額 = PSA10 参考価格 × 検証済み PSA10 鑑定枚数。毎日の更新ごとに再計算します。掲載条件は検証済み PSA10 鑑定枚数 1,000 枚以上。",
      methodLink: "算出方法",
      ...
      changeNote: "パーセント列は当該期間の PSA10 参考価格の変動で、鑑定枚数の再計算ではありません。",
      empty: "現在のスナップショットでは、このランキングに公表値を持つカードがありません。",
    },
    set: {
      kicker: "弾別 時価総額",
      h1: "{set} — カード時価総額ランキング（PSA10）",
      title: "{set} 時価総額ランキング（PSA10）",
      description: "CardZ Marketcap が収録する {set} の PSA10 カード {count} 枚の時価総額ランキング。PSA10 価格 × 鑑定枚数、合計 {total}、基準日 {date}。",
      summary: "CardZ Marketcap は {set} の PSA10 カードを {count} 枚収録しており、{date} 時点の合計時価総額は {total} です。時価総額は検証済み PSA10 鑑定枚数 × PSA10 参考価格。弾内の最大は {top} で {topValue} です。",
      tableCaption: "{set} の PSA10 時価総額ランキング（基準日 {date}）",
    ...
    rankings: {
      ...
      indexDescription: "ポケモンカードとワンピースカードの PSA10 時価総額・価格・鑑定枚数・値動きランキング。CardZ Marketcap のデータ、基準日 {date}。",
      indexSummary: "CardZ Marketcap は同一の日次スナップショットから、鑑定済みカード {count} 枚をもとに {lists} 本のランキングを公開しています。時価総額は検証済み PSA10 鑑定枚数 × PSA10 参考価格で、本ページの全ランキングは {date} 時点です。",
      listHeading: "ランキング一覧",
      siblings: "他のランキング",
      tableCaption: "ランキング（基準日 {date}）",
      titles: {
        value: "{game} 時価総額ランキング（{year}年）",
        price: "{game} 高額ランキング PSA10価格（{year}年）",
        gainers: "{window} 値上がりランキング PSA10価格（{year}年）",
        losers: "{window} 値下がりランキング PSA10価格（{year}年）",
        population: "PSA10 鑑定枚数ランキング — トレカ時価総額指数",
        rarity: "PSA10 鑑定枚数が少ないカード ランキング（{year}年）",
        sets: "{game} 弾別 時価総額ランキング（{year}年）",
      },
      h1: {
        value: "{game} 時価総額ランキング（{year}年）— PSA10 時価総額順",
        price: "{game} 高額ランキング（{year}年）— PSA10 参考価格順",
        gainers: "{window} 値上がりランキング（{year}年）— PSA10 価格の変動",
        losers: "{window} 値下がりランキング（{year}年）— PSA10 価格の変動",
        population: "PSA10 鑑定枚数が多いカード（{year}年）— トレカ時価総額指数",
        rarity: "PSA10 鑑定枚数が少ないカード（{year}年）— 価格のある最少鑑定枚数",
        sets: "{game} 弾別 時価総額ランキング（{year}年）— PSA10 指数",
      },
      descriptions: {
        value: "CardZ Marketcap が PSA10 時価総額（PSA10 価格 × 検証済み鑑定枚数）で並べた{game}ランキング。鑑定枚数と価格変動も表示、基準日 {date}。",
        price: "PSA10 参考価格で並べた{game}の高額ランキング。各カードに検証済み PSA10 鑑定枚数と時価総額を併記。CardZ Marketcap、基準日 {date}。",
        gainers: "ポケモンカードとワンピースカードの PSA10 価格{window}値上がりランキング。各カードに鑑定枚数と時価総額を併記。CardZ Marketcap、基準日 {date}。",
        losers: "ポケモンカードとワンピースカードの PSA10 価格{window}値下がりランキング。各カードに鑑定枚数と時価総額を併記。CardZ Marketcap、基準日 {date}。",
        population: "CardZ Marketcap の時価総額指数で、検証済み PSA10 鑑定枚数が最も多いカード。PSA10 価格と時価総額を併記、基準日 {date}。",
        rarity: "CardZ Marketcap で PSA10 参考価格があり、検証済み鑑定枚数が最も少ないカード。時価総額も併記、基準日 {date}。",
        sets: "CardZ Marketcap が合計 PSA10 時価総額で並べた{game}の全弾。カード数と弾内最大カードを併記、基準日 {date}。",
      },
      answers: {
        value: "CardZ Marketcap は{scope}のカードを PSA10 時価総額 —— 検証済み PSA10 鑑定枚数 × PSA10 参考価格 —— で並べています。{date} 時点の首位は {top}（{topValue}）で、本ページの {count} 枚の合計時価総額は {total} です。",
        price: "この CardZ Marketcap のランキングは、{scope}のカードを PSA10 参考価格（時価総額の価格側、価格 × PSA10 鑑定枚数）で並べたものです。{date} 時点の最高価格は {top} の {topValue}、本ページは {count} 枚を掲載しています。",
        gainers: "CardZ Marketcap は各カードの PSA10 参考価格の{window}変動を追跡しています。{date} 時点で{window}の上昇が最も大きいのは {top}（{topValue}）、本ページは {count} 枚を掲載。時価総額はこの価格 × PSA10 鑑定枚数です。",
        losers: "CardZ Marketcap は各カードの PSA10 参考価格の{window}変動を追跡しています。{date} 時点で{window}の下落が最も大きいのは {top}（{topValue}）、本ページは {count} 枚を掲載。時価総額はこの価格 × PSA10 鑑定枚数です。",
        population: "鑑定枚数とは PSA が 10 と判定した枚数です。{date} 時点で CardZ Marketcap 指数の検証済み PSA10 鑑定枚数が最も多いのは {top} の {topValue} 枚で、本ページは上位 {count} 件を掲載しています。時価総額はこの枚数 × PSA10 参考価格です。",
        rarity: "これは CardZ Marketcap 指数のうち、価格があり検証済み PSA10 鑑定枚数が最も少ないカードです。{date} 時点の最少は {top} の {topValue} 枚。枚数が少ないことは希少性であって時価総額の大きさではありません（時価総額 = 鑑定枚数 × PSA10 価格）。",
        sets: "CardZ Marketcap は{scope}のカードを弾ごとにまとめ、PSA10 時価総額を合計しています。{date} 時点の最大は {top}（{topValue}）で、本ページの {count} 弾の合計は {total} です。",
      },
    },
    report: {
      kicker: "マーケットレポート",
      ...
      description: "鑑定カード時価総額指数の月次サマリー。ポケモンとワンピースの PSA10 時価総額、集中度、値動きの大きいカードを掲載。基準日 {date}。",
      summary: "CardZ Marketcap はポケモンとワンピースの鑑定済みカード {count} 枚を PSA10 時価総額（検証済み PSA10 鑑定枚数 × PSA10 参考価格）で追跡しています。{date} 時点の指数合計は {total}：ポケモン {pokemon}、ワンピース {onePiece}。",
      statCards: "追跡カード数",
      statTotal: "PSA10 時価総額 合計",
      statPokemon: "ポケモン時価総額",
      statOnePiece: "ワンピース時価総額",
      statTop10: "上位 10 枚の構成比",
      totalsHeading: "時価総額の合計",
      totalsLine: "CardZ Marketcap によると、{date} 時点で指数に含まれる鑑定済みカード {count} 枚の PSA10 時価総額は合計 {total}（ポケモン {pokemon}、ワンピース {onePiece}）です。",
      concentrationHeading: "集中度",
      concentrationLine: "{date} 時点で上位 10 枚が指数の {share} を占め、金額は {topTen}（合計 {total}）です。単体最大は {top} の {topValue}。",
      gainersHeading: "値上がり首位",
      gainersLine: "{date} 時点で PSA10 参考価格の{window}上昇が最も大きいのは {top}（{topValue}）です。",
      losersHeading: "値下がり首位",
      losersLine: "{date} 時点で PSA10 参考価格の{window}下落が最も大きいのは {top}（{topValue}）です。",
      populationHeading: "鑑定枚数 最多",
      populationLine: "{date} 時点で検証済み PSA10 鑑定枚数が最も多いのは {top} の {topValue} 枚です。",
      setsHeading: "最大の弾",
      setsLine: "{date} 時点で合計 PSA10 時価総額が最も大きい弾は {top}（{topValue}）です。",
