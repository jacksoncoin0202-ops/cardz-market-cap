...
const ja: SiteCopy = {
  ...
  methodology: {
    eyebrow: "算出方法",
    h1: "CardZ Marketcap のトレカ時価総額の算出方法（PSA 10 価格 × 鑑定枚数）",
    answer:
      "CardZ Marketcap はトレカ時価総額を「PSA 10 参考価格 × 確認済み PSA 10 鑑定枚数」で算出します。1 枚 100 ドルで PSA 10 が 10,000 枚あれば時価総額は 1,000,000 ドルです。対象はポケモンカード時価総額とワンピースカード時価総額、データ基準日は {asOf} です。",
    scope:
      "ここでいう時価総額とは「最高鑑定グレードで現存する全枚数の総価値」であり、1 枚の取引価格ではありません。株価×発行済株式数と同じ考え方で、安くても鑑定枚数が膨大なカードは、高額でもほとんど鑑定に出されていないカードより上位になり得ます。",
    formula: {
      heading: "計算式",
      body: [
        "PSA 10 時価総額 ＝ PSA 10 参考価格 × PSA 10 鑑定枚数。",
        "PSA 10 参考価格は、まったく同じ版（プリンティング）の PSA 10 1 枚に対して当サイトが保持する価格です。PSA 10 鑑定枚数は、PSA が公表するポピュレーションレポートで、その版が 10 と判定された枚数です。",
    ...
    worked: {
      ...
      rows: [
        ...
        { label: "PSA 10 参考価格", value: "{topPrice}" },
        { label: "PSA 10 鑑定枚数", value: "{topPop}" },
        { label: "PSA 10 時価総額", value: "{topCap}" },
      ],
    ...
    eligibility: {
      ...
      body: [
        "統計的に意味のある規模の確認済み PSA 10 鑑定枚数と、当サイトが責任を持てる価格の両方が揃った版のみランキングに入ります。{asOf} 時点で、掲載カードの PSA 10 鑑定枚数の最小値は {minPop} です。",
    ...
    gradedVsRaw: {
      heading: "なぜ生カードではなく PSA 10 なのか",
      body: [
        "生カードには確認可能な供給量がありません。1999 年の基本セットが今何枚残っていて、どんな状態かは誰にも分かりません。つまり「生価格 × 印刷枚数」は数値の形をした推測にすぎません。",
        "PSA 10 鑑定枚数は数え上げられ公表された数字で、提出が増えるにつれ一方向にしか動きません。同じグレードの価格と組み合わせれば、両方の入力が観測可能で日付も付いた、検証できる時価総額になります。",
        "代償として、PSA 10 時価総額は生カードや低グレードが持つ価値を無視します。測っているのは最高グレード市場であり、趣味全体ではありません。",
      ],
    },
    limits: {
      heading: "隠さずに書く限界",
      body: [
        ...
        "時価総額は流動性ではありません。1 枚の参考価格で PSA 10 全枚数を売り切ることは誰にもできません。この数値が示すのは規模であり、換金額ではありません。",
      ],
    },
    compare: {
      heading: "価格だけの指数との違い",
      ...
      table: {
        ...
        rows: [
          ["CardZ Marketcap", "PSA 10 参考価格 × 確認済み PSA 10 鑑定枚数", "あり（PSA ポピュレーションレポート）"],
          ["価格のみの相場サイト", "1 枚の直近取引価格または平均価格", "供給量の項目自体がない"],
          ["CARDS という名の暗号トークン", "無関係なデジタル資産の価格", "トレカ関連の商品ではない"],
        ],
      },
      note: "30 ドルで PSA 10 が 300,000 枚あるカードは、30,000 ドルで 40 枚しかないカードより上位になります。価格のみのランキングではこれが見えません。この指数が存在する理由はまさにそこです。",
    },
    versioning: {
      heading: "バージョンと変更",
      body: [
    ...
    cite: {
      ...
      template: "CardZ Marketcap「CardZ Marketcap のトレカ時価総額の算出方法」PSA 10 価格 × PSA 10 鑑定枚数、データ基準日 {asOf}。",
    ...
    faq: [
      {
        q: "トレカの時価総額とは何ですか。",
        a: "トレカの時価総額とは、PSA 10 1 枚の価格に、現存する PSA 10 の枚数を掛けた値です。1 枚の取引価格ではなく、そのカードの最高グレード供給全体の価値を示します。",
      },
      {
        q: "ポケカの時価総額はどう計算していますか。",
        a: "対象となるポケモンカードの版の PSA 10 参考価格に、PSA のポピュレーションレポートで確認した同じ版の PSA 10 枚数を掛けます。価格も枚数も日次で更新し、観測日を付けています。",
      },
      {
        q: "なぜ印刷枚数ではなく PSA 10 鑑定枚数を使うのですか。",
        a: "印刷枚数はほとんど公表されず、現存枚数も分かりません。PSA 10 鑑定枚数は数え上げられ公表されているため検証できます。計算式の両辺を観測可能に保てます。",
      },
      {
        q: "時価総額はそのカードの価値額ということですか。",
        a: "違います。時価総額は PSA 10 全枚数を同時にその価格で評価した理論値です。全部を同じ価格で売ることは不可能なので、換金額ではなく規模と供給の指標として見てください。",
      },
      {
  ...
  faq: {
    eyebrow: "よくある質問",
    h1: "ポケモンカード時価総額の FAQ：PSA 10 時価総額の疑問に回答",
    answer:
      "CardZ Marketcap は「PSA 10 参考価格 × 確認済み PSA 10 鑑定枚数」でポケモンカード時価総額とワンピースカード時価総額をランキングしています。本ページでは数値の作り方、対象範囲、そして分からないことを説明します。データ基準日は {asOf} です。",
    groups: [
      {
        heading: "基本",
        items: [
          {
            ...
            a: "CardZ Marketcap は鑑定済みトレカの日次時価総額指数です。確認済み PSA 10 鑑定枚数に PSA 10 参考価格を掛けてポケモンとワンピースのカードを順位付けし、各スナップショットの基準日を公開しています。",
          },
          {
            q: "PSA 10 時価総額とは何ですか。",
            a: "PSA 10 時価総額とは、PSA 10 1 枚の価格に PSA が 10 と判定した枚数を掛けた値です。1 枚の価格ではなく、最高グレード供給に存在する価値の総量を表します。",
          },
          {
      ...
      },
      {
        ...
        items: [
          {
            q: "トレカの時価総額はどう計算しますか。",
            a: "時価総額は、同じ版の PSA 10 参考価格に確認済み PSA 10 鑑定枚数を掛けた値です。両方の入力に観測日が付き、どちらかが確認できない場合そのカードはランキングに入りません。",
          },
          {
            q: "鑑定枚数のデータはどこから来ますか。",
            a: "PSA が公表するポピュレーションレポートから取得し、セット・カード番号・言語・仕様が一致する版に突き合わせています。使うのは PSA 10 の枚数のみで、下位グレードは時価総額に含めません。",
          },
          {
      ...
      },
      {
        ...
        items: [
          {
            q: "どのカードが対象ですか。",
            a: "確認済み PSA 10 鑑定枚数があり、価格に責任を持てるポケモンとワンピースのシングルカードが対象です。{asOf} 時点で {ranked} 版を掲載しており、内訳はポケモン {pokemon} 版、ワンピース {onePiece} 版です。",
          },
          {
          ...
          {
            q: "未開封ボックスも対象ですか。",
            a: "未開封商品はシングルカードとは別に管理しています。PSA 10 鑑定枚数が存在しないためシングルの時価総額ランキングには入らず、その合計にも混ざりません。",
          },
          {
            q: "追跡中の時価総額の合計はいくらですか。",
            a: "{asOf} 時点で、公開指数は {ranked} 版について合計 {totalCap} の PSA 10 時価総額を追跡しています。この合計は価格の変動と、新たに鑑定された枚数によって日々変わります。",
          },
        ],
      },
      {
        ...
        items: [
          {
            ...
            a: "ランキングは随時公開され、毎日並び替わります。上位は高い PSA 10 価格と大きな鑑定枚数を併せ持つため、「最も高額な 1 枚」の一覧とはたいてい一致しません。",
          },
          {
            q: "なぜ安いカードが高いカードより上位なのですか。",
            a: "時価総額は価格に供給量を掛けるからです。30 ドルで PSA 10 が 300,000 枚あるカードは、1 枚の価格がはるかに安くても、30,000 ドルで 40 枚のカードより総額が大きくなります。",
          },
          {
            q: "記事や動画でこの数値を使えますか。",
  ...
  glossary: {
    eyebrow: "用語集",
    h1: "鑑定カード市場指数の用語集：PSA 10 時価総額の用語",
    answer:
      "この用語集は、CardZ Marketcap が鑑定カードを順位付けする際に使う用語を定義します。PSA 10 時価総額、ポピュレーションレポート、ジェムレート、確認済み供給量などが対象です。定義は {asOf} 公開の指数に適用されます。",
    setName: "CardZ Marketcap 鑑定カード用語集",
    setDescription: "CardZ Marketcap の鑑定カード市場指数で使用する、鑑定・価格・時価総額に関する用語の定義。",
    terms: [
      { id: "market-cap", term: "時価総額（トレカ）", def: "ある版の PSA 10 参考価格に確認済み PSA 10 鑑定枚数を掛けた値。1 枚の価格ではなく、最高グレード供給の総価値を示す。" },
      { id: "psa-10-market-cap", term: "PSA 10 時価総額", def: "PSA 10 の入力だけで算出した時価総額。PSA 10 1 枚の価格と PSA 10 の枚数を使い、下位グレードは含めない。" },
      { id: "psa", term: "PSA", def: "Professional Sports Authenticator。第三者鑑定会社で、提出されたカードにグレードを付け、グレードごとの枚数を公表する。" },
      { id: "psa-10", term: "PSA 10", def: "PSA の最高標準グレードで Gem Mint と呼ばれる。CardZ Marketcap は価格側と供給側の両方でこのグレードを使う。" },
      { id: "population-report", term: "ポピュレーションレポート", def: "PSA が公表する、各グレードで鑑定した枚数の集計。時価総額の供給項の出典。" },
      { id: "pop", term: "ポップ", def: "population の略。Pop 10 は、特定のセット・言語・仕様の版で PSA 10 と判定された枚数を指す。" },
      { id: "reference-price", term: "参考価格", def: "ある日付において、ある版の PSA 10 1 枚に対して CardZ Marketcap が保持する価格。同じ版の鑑定済み取引の証跡から導く。" },
      ...
      { id: "gem-rate", term: "ジェムレート", def: "提出されたカードのうち PSA 10 が付く割合。低いほど、提出が多くても PSA 10 の供給はゆっくりしか増えない。" },
      ...
      { id: "index", term: "鑑定カード市場指数", def: "鑑定データから算出した価値指標でカードを順位付けしたもの。CardZ Marketcap は PSA 10 の価格と鑑定枚数に基づく鑑定カード市場指数。" },
  ...
  about: {
    ...
    answer:
      "CardZ Marketcap は鑑定済みトレカの日次時価総額指数です。確認済み PSA 10 鑑定枚数に PSA 10 参考価格を掛けて、ポケモンとワンピースのカードを順位付けしています。公開中のスナップショットの基準日は {asOf} です。",
    descriptor:
    ...
    sections: [
      {
        heading: "公開しているもの",
        body: [
          "鑑定済みのポケモンとワンピースの版を PSA 10 時価総額で並べた日次ランキング。各行に価格・鑑定枚数・時価総額を表示します。",
      ...
      },
      {
        heading: "引用方法",
        body: [
          "数値を引用する際は CardZ Marketcap と明記し、参照元ページにリンクし、基準日を併記してください。例：CardZ Marketcap、PSA 10 時価総額、データ基準日 {asOf}。",
  ...
  data: {
    eyebrow: "データと API",
    h1: "CardZ Marketcap のデータと API：PSA 10 時価総額を JSON で取得",
    answer:
      "CardZ Marketcap は PSA 10 時価総額ランキングを、公開の読み取り専用 API から JSON で配信しています。各レスポンスにはスナップショットの generation、基準日、カードごとの価格・鑑定枚数・時価総額が含まれます。現在のスナップショットの基準日は {asOf} です。",
    endpoints: {
      heading: "エンドポイント",
