...
export default async function OnePiecePage() {
  const { snapshot, catalog } = firstPaintScope(scopeSnapshot(await loadMarketSnapshot(), "one-piece"));
  return <MarketPage kind="one-piece" snapshot={snapshot} catalog={catalog} />;
}
