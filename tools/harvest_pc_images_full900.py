...
Fail-closed: bind only when product_id maps to catalog_source_identity
  source_code=pricecharting AND match_status=exact.
...
def load_exact_pc_identities(cur) -> dict[str, int]:
    """product_id (external_entity_id) -> variant_id for exact PC binds only."""
    cur.execute(
        """
...
def harvest(args: argparse.Namespace) -> dict[str, Any]:
    ...
    cur = conn.cursor()
    try:
        exact = load_exact_pc_identities(cur)
        exact_count = len(exact)

        # Fail-closed: only exact PC identities
        ...
        stats: dict[str, Any] = {
            ...
            "exactIdentities": exact_count,
            "missing_exact_id": missing_exact_id,
        ...
        print(json.dumps({k: stats[k] for k in (
            ...
            "missing_exact_id", "jobsPlanned", "dbInserted", "dbPcAssets",
            "dbPcPublicAllowed", "exactIdentities", "scannedProductIds",
